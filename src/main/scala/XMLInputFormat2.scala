  ...
  (val split: FileSplit, val jobConf: JobConf) extends RecordReader[LongWritable, Text]{
    ...
    private def readUntilMatch(_match: Array[Array[Byte]], withinBlock: Boolean): Boolean = {
      ...
      while (true) {
        ...
        if (b == _match(i)) {
          i += 1
          if (i >= _match.length) return true
        }
        else i = 0
...
class XMLInputFormat2 extends TextInputFormat {
  @throws[IOException]
  override def getRecordReader(inputSplit: InputSplit, jobConf: JobConf, reporter: Reporter) = new XMLInputFormat2.XmlRecordReader(inputSplit.asInstanceOf[FileSplit], jobConf)
}
